...
summ.expr.by.grp <- function(dat, gns=NULL, mdat, grp, blk=NULL, exp=TRUE, f1=mean, t1=scale, f2=mean, t2=NULL, pct=TRUE, expr.cutoff=0, ncells.cutoff=3, ret.no.t=FALSE, ret.grp.sizes=FALSE, ret.rmv.summ=FALSE) {
  ...
  # mdat: cell metadata as a data.frame or data.table, with rows in the same order as the columns of `dat`; if `dat` is a Seurat object then will automatically use dat@meta.data and this will be ignored
  ...
  # t1: only effective if `blk` is given; transformation function applied across groups within each block after applying `f1`; set to NULL to skip this step
  # f2: only effective if `blk` is given; summary function applied to each group after applying `f1` and possibly `t1`; if a custom function, please remember to use na.rm=TRUE
  # t2: transformation function applied finally (regardless of if `blk` is given); set to NULL to skip this
  ...
  if (is.null(blk)) {
    ...
    if (identical(f1, mean)) {
      ...
      avg <- sapply(idxs, function(idx) sparseMatrixStats::rowMedians(dat[, idx, drop=FALSE], useNames=TRUE))
    } else {
      avg <- sapply(idxs, function(idx) apply(dat[, idx, drop=FALSE], 1, f1))
    }
    if (exp) avg <- log1p(avg)
    if (!is.null(t2)) {
      if (ret.no.t) avg1 <- avg
      if (identical(t2, scale)) {
        avg <- t(scale(t(avg)))
      } else {
        avg <- t(apply(avg, 1, t2))
      }
    }
    res <- list(avg=avg)
    if (pct) {
      res$pct <- 100*sapply(idxs, function(idx) sparseMatrixStats::rowMeans2(dat[, idx, drop=FALSE]>expr.cutoff, useNames=TRUE))
    }
    ...
    if (identical(f1, mean)) {
      ...
      avg <- lapply(idxs, function(js) sapply(js, function(j) sparseMatrixStats::rowMedians(dat[, j, drop=FALSE], useNames=TRUE)))
    } else {
      avg <- lapply(idxs, function(js) sapply(js, function(j) apply(dat[, j, drop=FALSE], 1, f1)))
    }
    if (!is.null(t1)) {
      if (exp) avg <- lapply(avg, log1p)
      if (ret.no.t) avg1 <- avg
      if (identical(t1, scale)) {
    ...
    if (identical(f2, mean)) {
      ...
      if (exists("avg1")) avg1 <- sapply(grps, function(j) matrixStats::rowMedians(do.call(cbind, lapply(avg1, function(x) x[, j[j %in% colnames(x)]])), na.rm=TRUE, useNames=TRUE))
    } else if (!is.null(f2)) {
      avg <- sapply(grps, function(j) apply(do.call(cbind, lapply(avg, function(x) x[, j[j %in% colnames(x)]])), 1, f2))
      if (exists("avg1")) avg1 <- sapply(grps, function(j) apply(do.call(cbind, lapply(avg1, function(x) x[, j[j %in% colnames(x)]])), 1, f2))
    } else {
      mdat.res <- data.table(block=rep(names(avg), sapply(avg, ncol)))
      avg <- do.call(cbind, avg)
      mdat.res[, group:=colnames(avg)]
      if (exists("avg1")) avg1 <- do.call(cbind, avg1)
    }
    if (exp && is.null(t1)) avg <- log1p(avg)
    if (!is.null(t2)) {
      if (ret.no.t && !exists("avg1")) avg1 <- avg
      if (identical(t2, scale)) {
        avg <- t(scale(t(avg)))
      } else {
        avg <- t(apply(avg, 1, t2))
    ...
    res <- list(avg=avg)
    if (pct) {
      pct <- lapply(idxs, function(js) sapply(js, function(j) sparseMatrixStats::rowMeans2(dat[, j, drop=FALSE]>expr.cutoff, useNames=TRUE)))
      if (identical(f2, mean)) {
